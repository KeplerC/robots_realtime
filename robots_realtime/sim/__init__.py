...
def make_eyeball_env(
    ...
    **kwargs,
    ...
    """Create a MuJoCo YAM environment with one of the ported task definitions.

    Args:
        task_name: Key in ``robots_realtime.sim.eyeball.tasks.TASK_REGISTRY``
            (e.g. ``"tape_handover_random"``, ``"pick_up_tiger"``).
        eyeball_xml: Path to the base scene XML. Defaults to the bundled
            ``eyeball_bimanual_scene.xml`` (bimanual YAM, no eye gimbal).
        render_cameras: Whether to render camera images in observations.
        config: Optional SimConfig override.
